...
if ( !class_exists( 'Ovic_Settings_Backup' ) ) {
	class  Ovic_Settings_Backup
	{
		...
		public function __construct()
		{
			...
			add_filter( 'ovic_settings_print_option_page', array( $this, 'ovic_settings_print_option_page' ), 100, 3 );
		...
		public function options( $options )
		{
			$options['ovic_settings_backup'] = array(
				...
				'title'      => __( 'Import/Export', 'bianco-toolkit' ),
				'show_names' => true,
				'sections'   => array(
		...
		function ovic_settings_print_option_page( $html, $active_tab, $active_section )
		{
			if ( $active_tab == 'ovic_settings_backup' ) {
				ob_start();
				?>
                <h3 class="title"><?php esc_html_e( 'Export Options', 'bianco-toolkit' ); ?></h3>
                <p><?php echo __( 'Click below to generate a <b>.txt</b> file for all settings.', 'bianco-toolkit' ); ?></p>
                <form action="" method="post">
                    <input type="hidden" name="ovic_action" value="export_options">
                    <button class="button button-primary"><?php esc_html_e( 'Export', 'bianco-toolkit' ); ?></button>
                </form>

                <h3 class="title"><?php esc_html_e( 'Import Options', 'bianco-toolkit' ); ?></h3>
                <form action="" method="post">
                    ...
                    <button class="button button-primary"><?php esc_html_e( 'Import', 'bianco-toolkit' ); ?></button>
                </form>
				<?php
				$html = ob_get_clean();
			}
