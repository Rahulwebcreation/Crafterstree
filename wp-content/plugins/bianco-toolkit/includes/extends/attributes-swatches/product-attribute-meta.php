...
if ( !class_exists( 'Ovic_Attribute_Product_Meta' ) ) {
	class Ovic_Attribute_Product_Meta
	{
		...
		{
			include_once dirname( __FILE__ ) . '/product-term.php';
		...
		{
            wp_register_style( 'product-attributes-swatches', $this->plugin_uri . 'product-attribute.css', array(), '1.0' );
            wp_register_script( 'product-attributes-swatches', $this->plugin_uri . 'product-attribute.js', array(), '1.0', true );
		}

		public function product_attributes_type_selector( $types )
		{
			$ovic_types = array(
				'box_style' => esc_html__( 'Box Style', 'bianco-toolkit' ),
			);
		...
		public function update_attribute( $id, $data )
		{
			if ( $data['attribute_type'] === 'box_style' ) {
				...
				$data['attribute_size']   = isset( $_POST['attribute_size'] ) ? $_POST['attribute_size'] : '40x40';
		...
		public function add_attribute_fields()
		{
            ...
            <div class="form-field" style="display: none;">
                <label><?php esc_html_e( 'Size Box', 'bianco-toolkit' ); ?></label>
                <input type="text" name="attribute_size" id="attribute_size" value="40x40">
                <p class="description"><?php esc_html_e( 'Determines Size in this attribute.{width}x{height}', 'bianco-toolkit' ); ?></p>
            </div>
		...
		public function edit_attribute_fields()
		{
			...
			if ( is_wp_error( $attribute ) )
				return;
			$attribute_size = isset( $attribute->attribute_size ) ? $attribute->attribute_size : '40x40';
			?>
            <tr class="form-field form-required" <?php if ( $attribute->attribute_type == 'select' ): ?> style="display: none;" <?php endif; ?>>
                <th scope="row" valign="top"><label><?php _e( 'Size Box', 'bianco-toolkit' ); ?></label></th>
                <td>
		...
		public function product_option_terms( $attribute_taxonomy, $i )
		{
			...
			if ( 'box_style' === $attribute_taxonomy->attribute_type ) : ?>
                <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'bianco-toolkit' ); ?>"
                ...
                <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'bianco-toolkit' ); ?></button>
                <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'bianco-toolkit' ); ?></button>
                <button class="button fr plus add_new_attribute"><?php esc_html_e( 'Add new', 'bianco-toolkit' ); ?></button>
			<?php
			endif;
		}
		...
		public function add_attr_field()
		{
            ...
            <div class="form-field term-display-type-wrap">
                <label for="display_type"><?php _e( 'Display type', 'bianco-toolkit' ); ?></label>
                <select name="<?php echo esc_attr( $id_type ); ?>" id="product_attribute_type_id">
                    <option value="-1"><?php echo esc_html__( 'None', 'bianco-toolkit' ); ?></option>
                    <option value="color"><?php echo esc_html__( 'Color', 'bianco-toolkit' ); ?></option>
                    <option value="photo" selected><?php echo esc_html__( 'Photo', 'bianco-toolkit' ); ?></option>
                    <option value="label"><?php echo esc_html__( 'Label', 'bianco-toolkit' ); ?></option>
                </select>
                <script type="text/javascript">
            ...
            <div class="form-field term-display-type-wrap swatch color" style="display: none;">
                <label><?php _e( '', 'bianco-toolkit' ); ?></label>
            ...
            <div class="form-field term-display-type-wrap swatch photo">
                <label><?php _e( 'Thumbnail', 'bianco-toolkit' ); ?></label>
                ...
                <div style="line-height: 60px;">
                    ...
                    <button type="button"
                            class="upload_image_button button"><?php _e( 'Upload/Add image', 'bianco-toolkit' ); ?></button>
                    <button type="button"
                            class="remove_image_button button"><?php _e( 'Remove image', 'bianco-toolkit' ); ?></button>
                </div>
                <script type="text/javascript">
                    ...
                    jQuery(document).on('click', '.upload_image_button', function (event) {
                        ...
                        file_frame = wp.media.frames.downloadable_file = wp.media({
                            title: '<?php _e( 'Choose an image', 'bianco-toolkit' ); ?>',
                            button: {
                                text: '<?php _e( 'Use image', 'bianco-toolkit' ); ?>'
                            },
                            multiple: false
                        });
		...
		public function edit_attr_field( $term, $taxonomy )
		{
            ...
            <tr class="form-field swatch color <?php echo $swatch_term->get_type() == 'color' ? 'active' : ''; ?>">
                <th scope="row" valign="top"><label><?php _e( '', 'bianco-toolkit' ); ?></label></th>
                <td>
		...
		public function product_attr_columns( $columns )
		{
			...
			$new_columns['thumb'] = __( 'Image', 'bianco-toolkit' );
		...
		public function wc_variation_attribute_options( $html, $args )
		{
			...
			$args                  = wp_parse_args( apply_filters( 'woocommerce_dropdown_variation_attribute_options_args', $args ), array(
					...
					'id'               => '',
					'class'            => '',
					'show_option_none' => __( 'Choose an option', 'bianco-toolkit' ),
				)
			);
			...
			$show_option_none_text = $args['show_option_none'] ? $args['show_option_none'] : __( 'Choose an option', 'bianco-toolkit' ); // We'll do our best to hide the placeholder, but we'll need to show something when resetting options.
		...
		public function get_product_attribute( $attribute )
		{
			...
			try {
				...
				if ( is_wp_error( $attribute ) || is_null( $attribute ) ) {
					throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_id', __( 'A product attribute with the provided ID could not be found', 'bianco-toolkit' ), 404 );
				}
				...
				$product_attribute['size'] = isset( $attribute->attribute_size ) ? $attribute->attribute_size : '40x40';
