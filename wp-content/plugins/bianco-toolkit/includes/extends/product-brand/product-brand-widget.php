...
if ( class_exists( 'WC_Widget' ) && !class_exists( 'WC_Widget_Product_Brand' ) ) {
	class WC_Widget_Product_Brand extends WC_Widget
	{
		...
		public function __construct()
		{
			...
			$this->widget_description = __( 'A list or dropdown of product brand.', 'bianco-toolkit' );
			$this->widget_id          = 'woocommerce_product_brand';
			$this->widget_name        = __( 'Product Brand', 'bianco-toolkit' );
			$this->settings           = array(
				'title'              => array(
					'type'  => 'text',
					'std'   => __( 'Product brand', 'bianco-toolkit' ),
					'label' => __( 'Title', 'bianco-toolkit' ),
				),
				'orderby'            => array(
					'type'    => 'select',
					'std'     => 'name',
					'label'   => __( 'Order by', 'bianco-toolkit' ),
					'options' => array(
						'order' => __( 'Brand order', 'bianco-toolkit' ),
						'name'  => __( 'Name', 'bianco-toolkit' ),
					),
				),
				'show_as'            => array(
					'type'    => 'select',
					'std'     => 'list',
					'label'   => __( 'Show as', 'bianco-toolkit' ),
					'options' => array(
						'list'     => __( 'List', 'bianco-toolkit' ),
						'dropdown' => __( 'Dropdown', 'bianco-toolkit' ),
						'logo'     => __( 'Logo', 'bianco-toolkit' ),
					),
				),
				'count'              => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => __( 'Show product counts', 'bianco-toolkit' ),
				),
				'hierarchical'       => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => __( 'Show hierarchy', 'bianco-toolkit' ),
				),
				'show_children_only' => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => __( 'Only show children of the current category', 'bianco-toolkit' ),
				),
				'hide_empty'         => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => __( 'Hide empty brand', 'bianco-toolkit' ),
				),
				'max_depth'          => array(
					'type'  => 'text',
					'std'   => '',
					'label' => __( 'Maximum depth', 'bianco-toolkit' ),
				),
			);
		...
		public function widget( $args, $instance )
		{
				...
				if ( jQuery().selectWoo ) {
					var wc_product_cat_select = function() {
						jQuery( '.dropdown_product_cat' ).selectWoo( {
							placeholder: '" . esc_js( __( 'Select a category', 'bianco-toolkit' ) ) . "',
							minimumResultsForSearch: 5,
							width: '100%',
							allowClear: true,
							language: {
								noResults: function() {
									return '" . esc_js( _x( 'No matches found', 'enhanced select', 'bianco-toolkit' ) ) . "';
								}
							}
						} );
					};
			...
			"
				...
				$list_args['show_option_none']           = __( 'No product brand exist.', 'bianco-toolkit' );
