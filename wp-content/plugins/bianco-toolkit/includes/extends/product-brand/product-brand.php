	...
	class Ovic_Admin_Taxonomies
	{
		...
		function permalink_settings_init()
		{
			...
			add_settings_field(
				...
				__( 'Product brand base', 'bianco-toolkit' ), // setting title
		...
		function taxonomy_slug_input()
		{
            ...
            <input name="ovic_taxonomy_brand_slug" type="text" class="regular-text code"
                   ...
                   placeholder="<?php echo _x( 'product-brand', 'slug', 'bianco-toolkit' ) ?>"/>
			<?php
		}
		...
		function register_product_taxonomy()
		{
			...
			register_taxonomy(
				...
				array(
					...
					'label'                 => __( 'Brands', 'bianco-toolkit' ),
					'labels'                => array(
						'name'              => __( 'Product brands', 'bianco-toolkit' ),
						'singular_name'     => __( 'Category', 'bianco-toolkit' ),
						'menu_name'         => _x( 'Brands', 'Admin menu name', 'bianco-toolkit' ),
						'search_items'      => __( 'Search brands', 'bianco-toolkit' ),
						'all_items'         => __( 'All brands', 'bianco-toolkit' ),
						'parent_item'       => __( 'Parent brand', 'bianco-toolkit' ),
						'parent_item_colon' => __( 'Parent brand:', 'bianco-toolkit' ),
						'edit_item'         => __( 'Edit brand', 'bianco-toolkit' ),
						'update_item'       => __( 'Update brand', 'bianco-toolkit' ),
						'add_new_item'      => __( 'Add new brand', 'bianco-toolkit' ),
						'new_item_name'     => __( 'New brand name', 'bianco-toolkit' ),
						'not_found'         => __( 'No brands found', 'bianco-toolkit' ),
					),
					'show_ui'               => true,
					'query_var'             => true,
					'capabilities'          => array(
		...
		public function add_brand_fields()
		{
            ...
            <div class="form-field term-thumbnail-wrap">
                <label><?php _e( 'Brand Logo', 'bianco-toolkit' ); ?></label>
		...
		public function edit_brand_fields( $term )
		{
			...
			?>
            <tr class="form-field">
                <th scope="row" valign="top"><label><?php _e( 'Brand Logo', 'bianco-toolkit' ); ?></label></th>
            ...
            <tr class="form-field">
                <th scope="row" valign="top"><label><?php _e( 'Thumbnail', 'bianco-toolkit' ); ?></label></th>
                <td class="field-image-select">
                    <div class="product_brand_thumbnail" style="float: left; margin-right: 10px;">
		...
		public function product_brand_columns( $columns )
		{
			...
			$new_columns['logo']  = __( 'Logo', 'bianco-toolkit' );
		...
		public function product_brand_row_actions( $actions, $term )
		{
			...
			if ( $default_brand_id !== $term->term_id && current_user_can( 'edit_term', $term->term_id ) ) {
				$actions['make_default'] = sprintf(
					...
					esc_attr( sprintf( __( 'Make &#8220;%s&#8221; the default brand', 'bianco-toolkit' ), $term->name ) ),
					__( 'Make default', 'bianco-toolkit' )
				);
			}
		...
		public function product_brand_column( $columns, $column, $id )
		{
			if ( 'thumb' === $column ) {
				...
				if ( $default_brand_id === $id ) {
					$columns .= wc_help_tip( __( 'This is the default brand and it cannot be deleted. It will be automatically assigned to products with no brand.', 'bianco-toolkit' ) );
				}
				...
				$columns .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Thumbnail', 'bianco-toolkit' ) . '" class="wp-post-image" height="48" width="48" />';
			}
			if ( 'logo' === $column ) {
				// Prepend tooltip for default brand.
				$default_brand_id = absint( get_option( 'default_product_brand', 0 ) );
				...
				// Prevent esc_url from breaking spaces in urls for image embeds. Ref: https://core.trac.wordpress.org/ticket/23605
				$image   = str_replace( ' ', '%20', $image );
				$columns .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Logo', 'bianco-toolkit' ) . '" class="wp-post-image" height="48" width="48" />';
			}
			if ( 'handle' === $column ) {
