(function ($) {
    ...
    $.fn.ovic_vertical_menu = function () {
        ...
        if ( _verticalMenu.length > 0 ) {
            ...
            .on('click', '.close-cate', function (e) {
                _blockNav.find('li.link-other').each(function () {
                    $(this).slideUp();
                });
    ...
    $.fn.ovic_init_carousel  = function () {
        $(this).not('.slick-initialized').each(function () {
            ...
            if ( _this.hasClass('slick-vertical') ) {
                ...
                var csn_nav = _this.data('custom_nav');
                if(typeof csn_nav !== 'undefined'){
                    if(typeof csn_nav.next !== 'undefined'){
                        _config.nextArrow = '<div class="next">'+csn_nav.next+'<span class="fa fa-angle-right next"></span></div>';
                    }else{
                        _config.nextArrow = '<span class="fa fa-angle-right next"></span>';
                    }
                    if(typeof csn_nav.pre !== 'undefined'){
                        _config.prevArrow = '<div class="prev"><span class="fa fa-angle-left prev"></span>'+csn_nav.pre+'</div>';
                    }else{
    ...
    $.fn.ovic_product_thumb  = function (add_config) {
        $(this).not('.slick-initialized').each(function () {
            var _this       = $(this),
                _config     = ovic_ajax_frontend.data_slick;
            if ( add_config ) {
                _config = add_config;
            }
            if ( _config !== "" ) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if ( response.success == 'ok' ) {
                        ...
                        $(document).trigger('bianco_ajax_tabs_complete',_tabID);
                    } else {
    ...
    $(document).on('click', '.product:not(.product-type-external) .single_add_to_cart_button', function (e) {
        ...
        if ( !$thisbutton.hasClass('disabled') ) {
            ...
            /*Ajax action.*/
            $.post(ovic_ajax_frontend.ovic_ajax_url.toString().replace('%%endpoint%%', 'ovic_add_cart_single_ajax'), data, function (response) {
    ...
    $(document.body).on('removed_from_cart', function (event, fragments, cart_hash, $thisbutton) {

        $thisbutton.ovic_add_notify(
    ...
    $body.on('added_to_wishlist', function (event, $thisbutton) {
        ...
        $thisbutton.ovic_add_notify(
            ovic_ajax_frontend.added_to_wishlist_text + '</br>' +
            '<a href="' + ovic_ajax_frontend.wishlist_url + '">' +
            ovic_ajax_frontend.browse_wishlist_text + '</a>'
        );
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if ( $('.flex-control-thumbs').length > 0 ) {
            $('.flex-control-thumbs').ovic_product_thumb();
        }
        if ( $('.lazy').length > 0 ) {
