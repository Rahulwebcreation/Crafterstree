        ...
        <li class="menu-item block-userlink ovic-dropdown">
			<?php if ( is_user_logged_in() ): ?>
                <a data-ovic="ovic-dropdown" class="woo-wishlist-link logged"
                   href="<?php echo esc_url( $myaccount_link ); ?>"><span class="fa fa-user icon"
                                                                          aria-hidden="true"></span><span
                            class="text"><?php echo esc_html( $currentUser->display_name ); ?></span></a>
				...
				<?php else: ?>
                    <ul class="sub-menu">
                        <li class="menu-item">
                            <a href="<?php echo wp_logout_url( get_permalink() ); ?>"><?php esc_html_e( 'Logout', 'bianco-toolkit' ); ?></a>
                        </li>
                    </ul>
				<?php endif;
			else: ?>
                <a class="woo-wishlist-link" href="<?php echo esc_url( $myaccount_link ); ?>"><span
                            class="fa fa-user icon"></span><span
                            class="text"><?php echo esc_html__( 'Login', 'bianco-toolkit' ); ?></span></a>
			<?php endif; ?>
        </li>
		<?php
	}
...
if ( !function_exists( 'ovic_search_form' ) ) {
	function ovic_search_form()
	{
		...
		$args = array(
			'show_option_none'  => esc_html__( 'All Categories', 'bianco-toolkit' ),
                ...
                <div class="form-content search-box results-search">
                    <div class="inner">
                        <input autocomplete="off" type="text" class="searchfield txt-livesearch input" name="s"
                               ...
                               placeholder="<?php echo esc_html__( 'I&#39;m searching for...', 'bianco-toolkit' ); ?>">
                    </div>
                </div>
	...
	function ovic_share_button( $post_id )
	{
        ...
        <div class="ovic-share-socials">
            <a class="twitter"
               ...
               title="<?php echo esc_attr__( 'Twitter', 'bianco-toolkit' ) ?>"
               onclick='window.open(this.href, "", "menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600");return false;'>
                <figure>
                    <img src="<?php echo OVIC_FRAMEWORK_URI . "assets/images/share-twitter.png" ?>"
                         alt="<?php echo esc_attr__( 'Twitter', 'bianco-toolkit' ) ?>">
                </figure>
            </a>
            <a class="facebook"
               href="<?php echo esc_url( $facebook ); ?>"
               title="<?php echo esc_attr__( 'Facebook', 'bianco-toolkit' ) ?>"
               onclick='window.open(this.href, "", "menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600");return false;'>
                <figure>
                    <img src="<?php echo OVIC_FRAMEWORK_URI . "assets/images/share-facebook.png" ?>"
                         alt="<?php echo esc_attr__( 'Facebook', 'bianco-toolkit' ) ?>">
                </figure>
            </a>
            <a class="googleplus"
               href="<?php echo esc_url( $google ); ?>"
               title="<?php echo esc_attr__( 'Google+', 'bianco-toolkit' ) ?>"
               onclick='window.open(this.href, "", "menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600");return false;'>
                <figure>
                    <img src="<?php echo OVIC_FRAMEWORK_URI . "assets/images/share-ggplus.png" ?>"
                         alt="<?php echo esc_attr__( 'Google+', 'bianco-toolkit' ) ?>">
                </figure>
            </a>
            <a class="pinterest"
               href="<?php echo esc_url( $pinterest ); ?>"
               title="<?php echo esc_attr__( 'Pinterest', 'bianco-toolkit' ) ?>"
               onclick='window.open(this.href, "", "menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600");return false;'>
                <figure>
                    <img src="<?php echo OVIC_FRAMEWORK_URI . "assets/images/share-pin.png" ?>"
                         alt="<?php echo esc_attr__( 'Pinterest', 'bianco-toolkit' ) ?>">
                </figure>
            </a>
        </div>
		<?php
...
if ( !function_exists( 'ovic_product_options' ) ) {
	...
	function ovic_product_options( $allow = 'Theme Option' )
	{
		...
		/*except list style*/
        if (array_key_exists('content-product-list', $product_options)) {
            unset($product_options['content-product-list']);
        }
...
if ( !function_exists( 'ovic_register_taxonomy' ) ) {
	function ovic_register_taxonomy( $taxonomy )
	{
		$labels = array(
			'name'                       => $taxonomy['name'],
			'singular_name'              => $taxonomy['name'],
			'search_items'               => sprintf( esc_html__( 'Search %s', 'bianco-toolkit' ), $taxonomy['name'] ),
			'popular_items'              => sprintf( esc_html__( 'Popular %s', 'bianco-toolkit' ), $taxonomy['name'] ),
			'all_items'                  => sprintf( esc_html__( 'All %s', 'bianco-toolkit' ), $taxonomy['name'] ),
			'parent_item'                => null,
			'parent_item_colon'          => null,
			'edit_item'                  => sprintf( esc_html__( 'Edit %s', 'bianco-toolkit' ), $taxonomy['name'] ),
			'update_item'                => sprintf( esc_html__( 'Update %s', 'bianco-toolkit' ), $taxonomy['name'] ),
			'add_new_item'               => sprintf( esc_html__( 'Add New %s', 'bianco-toolkit' ), $taxonomy['name'] ),
			'new_item_name'              => sprintf( esc_html__( 'New %s Name', 'bianco-toolkit' ), $taxonomy['name'] ),
			'separate_items_with_commas' => sprintf( esc_html__( 'Separate %s with commas', 'bianco-toolkit' ), $taxonomy['name'] ),
			'add_or_remove_items'        => sprintf( esc_html__( 'Add or remove %s', 'bianco-toolkit' ), $taxonomy['name'] ),
			'choose_from_most_used'      => sprintf( esc_html__( 'Choose from the most used %s', 'bianco-toolkit' ), $taxonomy['name'] ),
			'not_found'                  => sprintf( esc_html__( 'No %s found.', 'bianco-toolkit' ), $taxonomy['name'] ),
...
if ( !function_exists( 'ovic_register_post_type' ) ) {
	function ovic_register_post_type( $posttype )
	{
		$labels = array(
			'name'               => $posttype['name'],
			...
			'add_new'            => esc_html__( 'Add New', 'bianco-toolkit' ),
			'add_new_item'       => sprintf( esc_html__( 'Add New %s', 'bianco-toolkit' ), $posttype['name'] ),
			'new_item'           => sprintf( esc_html__( 'New %s', 'bianco-toolkit' ), $posttype['name'] ),
			'edit_item'          => sprintf( esc_html__( 'Edit %s', 'bianco-toolkit' ), $posttype['name'] ),
			'view_item'          => sprintf( esc_html__( 'View %s', 'bianco-toolkit' ), $posttype['name'] ),
			'all_items'          => sprintf( esc_html__( 'All %s', 'bianco-toolkit' ), $posttype['name'] ),
			'search_items'       => sprintf( esc_html__( 'Search %s', 'bianco-toolkit' ), $posttype['name'] ),
			'parent_item_colon'  => sprintf( esc_html__( 'Parent %s:', 'bianco-toolkit' ), $posttype['name'] ),
			'not_found'          => sprintf( esc_html__( 'No %s found.', 'bianco-toolkit' ), $posttype['name'] ),
			'not_found_in_trash' => sprintf( esc_html__( 'No %s found in Trash.', 'bianco-toolkit' ), $posttype['name'] ),
		);
		$args   = array(
			'labels'             => $labels,
			'description'        => '',
