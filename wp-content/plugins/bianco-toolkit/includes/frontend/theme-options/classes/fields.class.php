	...
	abstract class OVIC_Fields extends OVIC_Abstract
	{
		...
		public function element_debug()
		{
			...
			if ( !empty( $this->where ) && ( !empty( $this->field['debug'] ) || ( defined( 'OVIC_DEBUG' ) && OVIC_DEBUG ) ) ) {
				...
				$out .= "<pre>";
				$out .= "<strong>" . __( 'CONFIG', 'bianco-toolkit' ) . ":</strong>";
				$out .= "\n";
				ob_start();
				...
				if ( !empty( $this->field['id'] ) ) {
					$out .= "\n";
					$out .= "<strong>" . __( 'ID', 'bianco-toolkit' ) . ":</strong>";
					$out .= "\n";
					$out .= $this->field['id'];
					$out .= "\n";
					$out .= "\n";
					$out .= "<strong>" . __( 'USAGE', 'bianco-toolkit' ) . ":</strong>";
					$out .= "\n";
					...
					if ( isset( $value ) ) {
						$out .= "\n\n";
						$out .= "<strong>" . __( 'VALUE', 'bianco-toolkit' ) . ":</strong>";
						$out .= "\n";
						ob_start();
						var_export( $value );
						$out .= htmlspecialchars( ob_get_clean() );
		...
		public function element_after_multilang()
		{
			...
			if ( is_array( $this->multilang ) ) {
				...
				$out .= '</fieldset>';
				$out .= '<p class="ovic-text-desc">' . sprintf( __( 'You are editing language: ( <strong>%s</strong> )', 'bianco-toolkit' ), $this->multilang['current'] ) . '</p>';
			}
...
if ( !function_exists( 'ovic_load_field_classes' ) ) {
	...
	{
		foreach ( glob( OVIC_PLUGIN_DIR . '/fields/*/*.php' ) as $field ) {
