	...
	class OVIC_Options extends OVIC_Abstract
	{
		...
		public function validate_save( $request )
		{
			...
			if ( isset( $request['import'] ) && !empty( $request['import'] ) ) {
				...
				$add_errors[] = $this->add_settings_error( __( 'Success. Imported backup options.', 'bianco-toolkit' ), 'updated' );
			}
			// reset all options
			if ( isset( $request['resetall'] ) ) {
				$add_errors[] = $this->add_settings_error( __( 'Default options restored.', 'bianco-toolkit' ), 'updated' );
			...
			if ( isset( $request['restore'] ) && !empty( $section_id ) ) {
				...
				$add_errors[] = $this->add_settings_error( __( 'Default options restored for only this section.', 'bianco-toolkit' ), 'updated' );
			}
		...
		public function add_options_html()
		{
			...
			echo ( $this->settings['show_search'] ) ? '<div class="ovic-search"><input type="text" placeholder="' . __( 'Search option(s)', 'bianco-toolkit' ) . '" /></div>' : '';
			echo '</div>';
			echo '<div class="ovic-header-right">';
			echo '<div class="ovic-buttons">';
			submit_button( __( 'Save', 'bianco-toolkit' ), 'primary ovic-save' . $ajax_class, 'save', false, array( 'data-save' => __( 'Saving...', 'bianco-toolkit' ) ) );
			submit_button( __( 'Restore', 'bianco-toolkit' ), 'secondary ovic-restore ovic-confirm', $this->unique . '[restore]', false );
			if ( $this->settings['show_reset'] ) {
				submit_button( __( 'Reset All Options', 'bianco-toolkit' ), 'secondary ovic-reset ovic-warning-primary ovic-confirm', $this->unique . '[resetall]', false );
			}
			echo '</div>';
			echo ( empty( $has_nav ) && $this->settings['show_all_options'] ) ? '<a href="#" class="ovic-expand-all"><i class="fa fa-eye-slash"></i> ' . __( 'show all options', 'bianco-toolkit' ) . '</a>' : '';
			echo '</div>';
