	...
	class OVIC_Metabox extends OVIC_Abstract
	{
		...
		public function add_meta_box_content( $post, $callback )
		{
			...
			if ( !empty( $args['show_restore'] ) ) {
				...
				echo '<span class="button ovic-button-restore">' . __( 'Restore', 'bianco-toolkit' ) . '</span>';
				echo '<span class="button ovic-button-cancel">' . sprintf( '<small>( %s )</small> %s', __( 'update post for restore ', 'bianco-toolkit' ), __( 'Cancel', 'bianco-toolkit' ) ) . '</span>';
				echo '</label>';
				echo '</div>';
			}
			echo '</div>';
