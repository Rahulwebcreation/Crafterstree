...
if ( !function_exists( 'ovic_get_icons' ) ) {
	function ovic_get_icons()
	{
		...
		$jsons = array_merge(
			...
			glob( OVIC_PLUGIN_DIR . '/fields/icon/*.json' )
		);
		if ( !empty( $jsons ) ) {
			foreach ( $jsons as $path ) {
				...
				if ( is_object( $object ) ) {
					...
					echo '<h4 class="ovic-icon-title">' . __( 'Error! Can not load json file.', 'bianco-toolkit' ) . '</h4>';
				}
			}
		}
                ...
                <div class="ovic-modal-table-cell">
                    ...
                    <div class="ovic-modal-inner">
                        <div class="ovic-modal-title">
							<?php _e( 'Add Icon', 'bianco-toolkit' ); ?>
                            <div class="ovic-modal-close ovic-icon-close"></div>
                        </div>
                        <div class="ovic-modal-header ovic-text-center">
                            <input type="text" placeholder="<?php _e( 'Search a Icon...', 'bianco-toolkit' ); ?>"
                                   class="ovic-icon-search"/>
                        </div>
                        <div class="ovic-modal-content">
                            <div class="ovic-icon-loading"><?php _e( 'Loading...', 'bianco-toolkit' ); ?></div>
                        </div>
                    </div>
                </div>
            </div>
