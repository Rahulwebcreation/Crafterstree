...
if ( !function_exists( 'ovic_add_field' ) ) {
	function ovic_add_field( $field = array(), $value = '', $unique = '', $where = '' )
	{
		...
		if ( !empty( $field['_notice'] ) ) {
			...
			$field            = array();
			$field['content'] = sprintf( __( 'Ooops! This field type (%s) can not be used here, yet.', 'bianco-toolkit' ), '<strong>' . $field_type . '</strong>' );
			$field['type']    = 'notice';
			$field['class']   = 'warning';
		}
		$output     = '';
		...
		if ( class_exists( $class ) ) {
			...
			$output .= '<p>' . __( 'This field class is not available!', 'bianco-toolkit' ) . '</p>';
		}
