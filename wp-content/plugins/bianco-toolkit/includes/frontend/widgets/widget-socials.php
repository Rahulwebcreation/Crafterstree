			...
			$array_settings           = apply_filters( 'ovic_filter_settings_widget_socials',
				array(
					...
					'ovic_socials' => array(
						'type'    => 'checkbox',
						'class'   => 'horizontal',
						'title'   => esc_html__( 'Select Social', 'bianco-toolkit' ),
						'options' => ovic_social_option(),
					),
				)
			);
			$this->widget_cssclass    = 'widget-ovic-socials';
			$this->widget_description = esc_html__( 'Display the customer Socials.', 'bianco-toolkit' );
			$this->widget_id          = 'widget_ovic_socials';
			$this->widget_name        = esc_html__( 'Ovic: Socials', 'bianco-toolkit' );
