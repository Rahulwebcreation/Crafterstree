	...
	class Ovic_VC_Shortcode
	{
		...
		public function __construct()
		{
			...
			if ( !empty( $this->shortcode ) ) {
				$this->enqueue = "ovic_shortcode_{$this->shortcode}";
		...
		function get_template( $template_name, $args = array(), $template_path = '', $default_path = '' )
		{
			...
			if ( !file_exists( $located ) ) {
				echo '<code>' . __( 'File does not exist.', 'bianco-toolkit' ) . '</code>';
		...
		public function save_post( $post )
		{
			...
			$wpdb->update(
				$wpdb->posts,
				array(
					'post_content' => $data->post_content,    // string
				),
				array(
					'ID' => $data->ID,
				),
		...
		public function add_css_editor( $atts, $tag )
		{
			...
			$editor_names = Ovic_VC_Init::ovic_vc_responsive_data();
			/* generate main css */
