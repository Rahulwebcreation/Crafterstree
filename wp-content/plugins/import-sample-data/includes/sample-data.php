<?php
if (!class_exists('Import_Sample_Data_Sample_Data')){
    class Import_Sample_Data_Sample_Data{
        /**
        ...
        public static function initialize() {
            ...
            self::$demo_site_pattern = Import_Sample_Data_Settings::demo_site_pattern();
            self::$theme_option_key  = Import_Sample_Data_Settings::theme_option_key();
            // Export data
        ...
        public static function uninstall_sample_data(){
            ...
            if ( ! isset( $_REQUEST['security'] ) || ! wp_verify_nonce( $_REQUEST['security'], 'import_sample_data_ajax_admin' ) ) {
                wp_send_json_error(esc_html__( 'Nonce verification failed. This might due to your working session has been expired. Please reload the page to renew your working session.', 'import-sample-data' ));

            }

            // Get selected sample data package.
            if ( ! isset( $_REQUEST['package'] ) ) {
                wp_send_json_error( esc_html__( 'Missing sample data package to install.', 'import-sample-data' ) );
            }
            ...
            if ( ! $package ) {
                wp_send_json_error( esc_html__( 'Failed to get data for the selected sample data package.', 'import-sample-data' ) );
            }
            ...
            if( count($backup)){
                ...
                foreach ( ( array ) $backup as $file ) {
                    ...
                    if ( false === $wpdb->query( 'COMMIT;' ) ) {
                        ...
                        wp_send_json_error(
                            sprintf(
                                esc_html__( 'Restoring backup has encountered an error and cannot continue: %s', 'import-sample-data' ),
                ...
                wp_send_json_error(esc_html__('Not found any backup to restore.','import-sample-data'));
            }
        ...
        public static function install_sample_data(){
            ...
            // Get selected sample data package.
            if ( ! isset( $_REQUEST['package'] ) ) {
                wp_send_json(esc_html__( 'Missing sample data package to install.', 'import-sample-data' ) );
            }
            $package = self::get_sample_package($_REQUEST['package'] );
        ...
        public static function import_sample_package($package){
            ...
            if (!is_dir($path)){
                ...
                if ( ! is_dir($path) ) {
                    wp_send_json_error(esc_html__( 'Failed to extract downloaded package to file system.', 'import-sample-data' ));
                }
            }else{
            ...
            if ( ! count( $sql ) ) {
                wp_send_json_error( esc_html__( 'Invalid sample data package.', 'import-sample-data' ) );
            }
            ...
            if( $option == 'full'){
                if( self::backup_database($package)){
                    ...
                    wp_send_json_error(esc_html__('The backup process error.','import-sample-data'));
                }
            }else{
                wp_send_json_error(esc_html__('Invalid parameters.','import-sample-data'));
            }
        ...
        public static function install_plugin(){
            ...
            if(empty($plugin)){
                wp_send_json_error( esc_html__( 'No plugin specified.', 'import-sample-data' ) );
            }
            // Get All Plugins
            $installed_plugins = get_plugins();
            // Install Plugins
            $plugins = Import_Sample_Data_Settings::plugins();
            if( !empty($plugins)){
            ...
            if ( is_wp_error( $result ) ) {
                wp_send_json_error( sprintf( esc_html__( 'Failed to activate %s plugin', 'import-sample-data' ),$plugin['name']));
            }
        ...
        public static function download_sample_package($package){
            ...
            if ( ! self::download( $package['download'],$path_z) ) {
                $error[] = sprintf(
                    esc_html__( 'Failed to download the selected sample data package &#39;%s&#39;.', 'import-sample-data' ),
                    $package['id']
                );
            }
                ...
                case 'install' :
                    ...
                    <div id="sample-data-installation-step-1">
                        <div class="notice-warning settings-error notice">
                            <strong class="label label-danger"><?php esc_html_e( 'Important Notice', 'import-sample-data' ); ?></strong>
                            <ul>
                                <li><?php printf( __( 'Installing sample data will replace the content of current website with <a href="%1$s" target="_blank" rel="noopener noreferrer"><strong>%2$s</strong> demo</a>.', 'import-sample-data' ), $package['demo'], $package['name'] ); ?></li>
                                <li><?php esc_html_e( 'You can later uninstall sample data to restore the original data back.', 'import-sample-data' ); ?></li>
                            </ul>
                        </div>
                        <?php
                            ...
                            <div id="sample-data-installation-options" class="import-sample-data-wrap">
                                ...
                                <div class="radio">
                                    <label>
                                        ...
                                        <?php esc_html_e( 'Extend Settings', 'import-sample-data' ); ?>
                                    </label>
                                </div>
                                <div class="box-wrap three-col select-page" style="display: none;">
                                    <p><?php esc_html_e('Select demo that you want to install.','import-sample-data');?></p>
                                    <?php foreach ( ( array ) $package['sample-page'] as $id => $page ) : ?>
                                        <div class="col">
                                            <div class="box">
                                                <a href="javascript:void(0)">
                                                    <img src="<?php echo esc_url( $page['thumbail'] ); ?>" alt="<?php echo esc_attr($page['name']);?>" />
                                                </a>
                                                <div class="box-info">
                        ...
                        <div class="checkbox">
                            <label>
                                ...
                                <?php esc_html_e( 'I understand the impact of installing sample data.', 'import-sample-data' ); ?>
                            </label>
                        </div>
                        <div class="installation-actions">
                            <button data-package="<?php echo esc_attr($package['id']);?>" id="config-action" disabled="disabled" class="button button-primary button-install"><?php esc_html_e('Continue','import-sample-data');?></button>
                            <button id="cancel-action" class="button button-cancel"><?php esc_html_e('Cancel','import-sample-data');?></button>
                        </div>

                    </div>
                    <div id="sample-data-installation-step-2" class="hidden">
                        <p>
                            <?php esc_html_e( 'There are several stages involved in the process. Please be patient.', 'import-sample-data' ); ?>
                        </p>
                        <ul id="install-processes">
                            <li id="install-sample-data-download-package">
                                <span class="title"><?php esc_html_e( 'Download sample data package.', 'import-sample-data' ); ?></span>
                                <span class="spinner is-active"></span>
                            </li>
                            <li id="install-sample-data-import-data" class="hidden">
                                <span class="title"><?php esc_html_e( 'Install sample data.', 'import-sample-data' ); ?></span>
                                <span class="spinner is-active"></span>
                            </li>
                            <li id="install-sample-data-required-plugins" class="hidden">
                                <span class="title"><?php esc_html_e( 'Install required plugins.', 'import-sample-data' ); ?></span>
                                <span class="spinner is-active"></span>
                    ...
                    <div id="install-sample-data-success-message" class="success-message hidden">
                        <h3>
                            <?php esc_html_e( 'Sample data was successfully installed.', 'import-sample-data' ); ?>
                        </h3>
                        <p>
                            <a href="<?php echo esc_url(  get_home_url() ); ?>" class=""><?php esc_html_e( 'Visit Site', 'import-sample-data' ); ?></a>
                    ...
                    <div id="install-sample-data-failure-message" class="failure-message hidden">
                        <h3>
                            <?php esc_html_e( 'Sample data was not successfully installed.', 'import-sample-data' ); ?>
                        </h3>
                    </div>
                    <?php
                    break;
                    ...
                    <div id="sample-data-uninstallation-step-1">
                        <div class="notice-warning settings-error notice">
                            ...
                            <ul>
                                ...
                                <li><?php esc_html_e( 'Uninstalling sample data will restore the backed up original data before the current sample data was installed..', 'import-sample-data' ); ?></li>
                            </ul>
                        </div>
                        <div class="checkbox">
                            <label>
                                <input name="agree" value="1" id="confirm-sample-data-installation" type="checkbox">
                                <?php esc_html_e( '	I understand the impact of uninstalling sample data.', 'import-sample-data' ); ?>
                            </label>
                        </div>
                        <div class="uninstallation-actions">
                            <button data-package="<?php echo esc_attr($package['id']);?>" id="config-action" disabled="disabled" class="button button-primary button-uninstall"><?php esc_html_e('Continue','import-sample-data');?></button>
                            <button id="cancel-action" class="button button-cancel"><?php esc_html_e('Cancel','import-sample-data');?></button>
                        </div>
                    </div>
                    <div id="sample-data-uninstallation-step-2" class="hidden">
                        ...
                        <ul id="install-processes">
                            <li id="uninstall-sample-data">
                                <span class="title"><?php esc_html_e( 'Uninstall', 'import-sample-data' ); ?></span>
                                <span class="spinner is-active"></span>
                            </li>
                        </ul>
                    </div>
        ...
        public static function export_sample_data(){
            if( isset($_REQUEST['import_sample_data_action']) && $_REQUEST['import_sample_data_action'] =='export-sample-data'){
                ...
                if( wp_verify_nonce($nonce,'export-sameple-data-form') && $backup_filename!=""){
                    ...
                    if ( ! self::prepare_directory( $backup_dir ) ) {
                        wp_send_json_error(  esc_html__( 'Failed to create directory to store database backup file.', 'import-sample-data' ) );
                    }
                    $exclude_table = array(
                        'options',
                        'users',
