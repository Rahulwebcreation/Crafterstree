 ...
 * @var $atts
 * Shortcode class
 * @var $this "Shortcode_Ovic_Map"
 * @version 1.0.0
 */
if ( !class_exists( 'Shortcode_Ovic_Map' )) {
    class Shortcode_Ovic_Map extends Ovic_VC_Shortcode
    {
		/**
		 * Shortcode name.
		 *
		...
		static public function add_css_generate( $atts )
		{
			...
			extract( $atts );
			$css = '';
			return apply_filters( 'Shortcode_Ovic_Map_css', $css, $atts );
		}
		...
		public function output_html( $atts, $content = null )
		{
                ...
                window.addEventListener('load',
                    function (ev) {
                        ...
                        var contentString = '<div style="background-color:#fff; padding: 30px 30px 10px 25px; width:290px;line-height: 22px" class="ovic-map-info">' +
                            ...
                            '<div class="map-field"><i class="fa fa-phone"></i><span><a href="tel:<?php echo esc_attr( $atts['phone'] ) ?>"><?php echo esc_html( $atts['phone'] ) ?></a></span></div>' +
                            '<div class="map-field"><i class="fa fa-envelope"></i><span><a href="mailto:<?php echo esc_attr( $atts['email'] ) ?>"><?php echo esc_html( $atts['email'] ) ?></a></span></div> ' +
                            '</div>';
			...
			<?php
			$html = ob_get_clean();
			return apply_filters( 'Shortcode_Ovic_Map', $html, $atts, $content );
		}
	}
	new Shortcode_Ovic_Map();
}